...
"""Basically the same as pyFlac_implementation.py but with key changes in:
        -constructor function (~line 14): passes audio_data numpy array instead of
                                        input file
        -encode function (~line 57-58): saves queue with bytes into a list and then 
                                        prints it to the payload.bin
 """

class FlacCodec:


    def __init__(self, audio_data, is_encode): #, output_name): # initialises encoder/decoder using StreamEncoder pyFlac function
    ...
    def encode(self): #encodes audio data
            ...
            self.encoder.finish()
            binary_data=list(self.queue.queue)
            print(binary_data)
            return binary_data
    ...
    def decoder_callback(self,
        ...
        output_file = sf.SoundFile(sys.stdout.buffer, mode="w",
