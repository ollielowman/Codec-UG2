...
def isBinary(data):
    ...
    Check if the given data is binary or not.
      ...
      Args:
          data: bytes-like object containing data to check.
...
def isWav(data):
    """
    Check if the given binary data corresponds to a .wav file based on its header.

      Args:
          data: bytes-like object containing data to check.

      Returns:
          bool: True if the data appears to be a .wav file, False otherwise.
...
def main():
    # Calls setup.py to download and/or install dependencies
    ...
    if isBinary(data):
        # Check if the binary data corresponds to a .wav file
        fileType = "wav" if isWav(data) else "bin"
